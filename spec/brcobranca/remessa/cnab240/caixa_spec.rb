  ...
  context 'convenio com 6 dígitos' do
    ...
    context 'validacoes' do
      ...
      context '@convenio' do
        ...
        it 'deve ser invalido se o convenio tiver mais de 7 digitos' do
          caixa.convenio = '12345678'
          expect(caixa.invalid?).to be true
          expect(caixa.errors.full_messages).to include('Convenio deve ter até 7 dígitos.')
        end
      end
    ...
    context 'geracao remessa' do
      ...
      context 'arquivo' do
        it { expect(caixa.gera_arquivo).to eq(read_remessa('remessa-caixa-cnab240-6.rem', caixa.gera_arquivo)) }
      end
    end
  end

  context 'convenio com 7 dígitos' do
    let(:params) do
      { empresa_mae: 'SOCIEDADE BRASILEIRA DE ZOOLOGIA LTDA',
        agencia: '12345',
        conta_corrente: '1234',
    ...
    context 'formatacao' do
      ...
      it 'versao do layout do arquivo deve retornar 107 para convênio até 7 dígitos' do
      ...
      it 'convenio lote deve retornar as informacoes nas posicoes corretas' do
        conv_lote = caixa.convenio_lote
        expect(conv_lote[0..6]).to eq '7654321'
      ...
      it 'info_conta deve retornar as informacoes nas posicoes corretas' do
        ...
        expect(info_conta[0..4]).to eq '12345' # agencia
        expect(info_conta[5]).to eq '1' # digito agencia
        expect(info_conta[6..12]).to eq '7654321' # convenio
      end
      ...
      it 'complemento P deve retornar as informacoes nas posicoes corretas' do
        comp_p = caixa.complemento_p pagamento
        expect(comp_p.size).to eq 34
        expect(comp_p[0..6]).to eq '7654321' # convenio
        expect(comp_p[17..18]).to eq '14' # modalidade carteira
        expect(comp_p[19..33]).to eq '000000000000123' # nosso numero
      end
    ...
    context 'geracao remessa' do
      ...
      context 'arquivo' do
        it { expect(caixa.gera_arquivo).to eq(read_remessa('remessa-caixa-cnab240-7.rem', caixa.gera_arquivo)) }
      end
    end
  end
end
